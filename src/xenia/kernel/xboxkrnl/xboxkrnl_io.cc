...
dword_result_t IoSynchronousDeviceIoControlRequest_entry(
  ...
  if (control_code == 0x4D014 && input_buffer_length == 36) {
    uint32_t* ptr_ptr_out = ctx.TranslateVirtual<uint32_t*>(input_buffer.guest_address() + 0x10);
    uint32_t dvd_drive_query_output_ptr = xe::load_and_swap<uint32_t>(ptr_ptr_out);
    char* put_6_here = ctx.TranslateVirtual<char*>(dvd_drive_query_output_ptr + 0xA);
    *put_6_here = 6;
  }
